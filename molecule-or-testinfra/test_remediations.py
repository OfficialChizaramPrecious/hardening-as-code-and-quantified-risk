...
These tests assume a hardened host. Rollback verification lives in
test_rollback.py, which runs at a different point in the lifecycle.

Two privilege and quoting notes:

* testinfra's host.file() runs unprivileged. Paths inside root-only
  directories - notably /etc/ssh/sshd_config.d, which is mode 0700 - must be
  read through sudo, or the file appears absent when it is not.
...
def test_kernel_parameters_are_persisted(host):
    ...
    persisted = host.file(HARDENING_SYSCTL_FILE)
    assert persisted.exists, "no persisted sysctl declaration"
    for key in HARDENED_SYSCTL_VALUES:
        assert key in persisted.content_string, f"{key} not persisted"
...
def test_accept_redirects_is_persisted_unlike_baseline(host):
    ...
    content = host.file(HARDENING_SYSCTL_FILE).content_string.replace(" ", "")
...
def test_spoofed_loopback_sources_dropped(firewalld_state):
    """HR-011, second half. This is the anti-spoofing measure the rule
    actually asks for: loopback addresses arriving on other interfaces."""
...
def test_firewall_control_did_not_break_loopback(host):
    """The paired assertion. Applying only the restriction half of HR-011
    blocks 127.0.0.1 and kills the declared service."""
    result = host.run("curl --fail --silent http://127.0.0.1:8080/")
